...
VPA_CONFIG = {
    "name": "VPA",
    "volume_spike_mult": 2.0,  # Lowered from 2.5x — forex volume thinner than crypto
    ...
    "candle_confirm": True,
    "bear_filter": False,      # VPA trades both directions regardless of regime
}

BREAKOUT_CONFIG = {
    "name": "Breakout",
    "consolidation_candles": 8,
    "consolidation_pips": 15,  # Raised from 12 — wider range acceptable for forex
    "breakout_volume_mult": 1.5,  # Lowered from 1.8 — forex off-hours lower volume
    "breakout_candle_close_ratio": 0.65,
    "min_breakout_pips": 3,  # Lowered from 4 — easier to trigger
    "time_filter": False,       # 24/7
    "candle_confirm": True,
    "bear_filter": False,      # Breakout trades both directions regardless of regime
}
...
def near_sr_level(price, symbol, bars, tolerance_pips=5):
    """Check if price is near a key S/R level"""
    pv = pip_value(symbol)
    tolerance = tolerance_pips * pv
    resistance, support = find_sr_levels(bars)
    for level in support:
        if abs(price - level) <= tolerance:
            return True, "NEAR_SUPPORT"
    for level in resistance:
        if abs(price - level) <= tolerance:
            return True, "NEAR_RESISTANCE"
    return False, ""
...
def run_ema(symbol, regime):
    ...
    try:
        ...
        # Higher timeframe alignment — 1H and 4H must agree
        bars_4h = get_candles(symbol, "H4", 30)
        closes_4h = [b["close"] for b in bars_4h] if bars_4h else []
        ema20_4h = calc_ema(closes_4h, 20) if len(closes_4h) >= 20 else []
        ema50_4h = calc_ema(closes_4h, 50) if len(closes_4h) >= 50 else []

        htf_bullish = (
            price > ema50_1h[-1] and
            (not ema50_4h or price > ema50_4h[-1])
        )

        score = 0
        if htf_bullish: score += 2  # Higher timeframe alignment bonus
        elif price > ema50_1h[-1]: score += 1  # At least 1H aligned
        ...
        # EMA pullback entry — price near EMA 21 is ideal entry
        pv = pip_value(symbol)
        dist_from_ema21 = abs(price - ema21[-1]) / pv
        near_ema21 = dist_from_ema21 <= 8  # Within 8 pips of EMA 21
        if near_ema21 and ema9[-1] > ema21[-1]:
            score += 1  # Bonus for pullback to EMA 21 in uptrend

        # S/R zone bonus
        bars_for_sr = get_candles(symbol, "M5", 80)
        sr_near, sr_type = near_sr_level(price, symbol, bars_for_sr)
        if sr_near and sr_type == "NEAR_SUPPORT":
            score += 1  # Near support = stronger long entry
        ...
        sig = {
            ...
            "regime_ok": regime != "BEAR", "htf_bullish": htf_bullish,
            "near_ema21": near_ema21, "sr_near": sr_near, "sr_type": sr_type,
...
def try_entry(symbol, strategy, sig, regime, now):
    ...
    if strategy == "EMA":
        ...
        # VPA trades regardless of regime — volume tells its own story

    elif strategy == "Breakout":
        if not sig.get("buy_signal"): return
        # Breakout trades regardless of regime — price action is direction
...
def trading_loop():
    ...
    add_diary("SYSTEM",
        "ForexAI Combined Bot v2.0 started | "
        "4 Strategies: EMA + MSS + VPA + Breakout | "
        "EUR/USD + GBP/USD | Units=5000 | SL=12pips TP=22pips | "
        "VPA threshold=2.0x | Bear filter removed VPA+Breakout | "
        "RSI direction check | HTF alignment | S/R zones | EMA pullback", "system")
    log.info("ForexAI Combined Bot v2.0 started")
